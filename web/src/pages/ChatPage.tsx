...
export function ChatPage() {
  ...
  useEffect(() => {
    if (routeChatId && routeChatId !== chatId) {
      loadChat(routeChatId).catch((e) => toast({ type: 'error', message: '加载对话失败：' + (e?.message || '') }));
    } else if (!routeChatId && !chatId && !sending) {
      // fresh chat — start one silently
      initChat('').catch((e) => toast({ type: 'error', message: '启动对话失败：' + (e?.message || '') }));
    }
  ...
  const handleNewChat = async () => {
    reset();
    navigate('/chat');
    try { await initChat(''); } catch { /* ignore */ }
  };
